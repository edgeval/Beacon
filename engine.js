...
class BeaconEngine {
    ...
    constructor(options = {}) {
        this.timeout = BeaconEngine._toInteger(
            options.timeoutMs,
            5000,
            500,
            30000
        ...
        this.concurrency = BeaconEngine._toInteger(
            options.concurrency,
            3,
            1,
            12
        ...
        this.pingCount = BeaconEngine._toInteger(
            options.pingCount,
            3,
            1,
            10
        ...
        this.protocolMode = ["https", "http", "auto"].includes(mode)
            ? mode
            : "https";

        const probeMode = String(options.probeMode || "auto").toLowerCase();

        this.probeMode = ["auto", "cors", "opaque"].includes(probeMode)
            ? probeMode
            : "auto";
        ...
        const currentWeight = BeaconEngine._toNumber(
        ...
        const historyWeight = BeaconEngine._toNumber(
        ...
        this.timeoutPenalty = BeaconEngine._toNumber(
            options.timeoutPenalty,
            500,
            0,
            10000
        );
    ...
    _parseTarget(domain) {
        ...
        const candidate = hasScheme
            ? raw
            : `${this._getDefaultProtocol()}//${raw}`;

        const url = new URL(candidate);

        if (!BeaconEngine._isHttpUrl(url)) {
            throw new TypeError(
                "Only HTTP and HTTPS routes are supported."
            );
        }

        if (url.username || url.password) {
            throw new TypeError(
                "Routes containing credentials are not supported."
            );
        }
    ...
    async _pingSingle(domain, path, fetchMode) {
        ...
        const probeUrl = this._buildProbeUrl(
            domain,
            path
        );
    ...
    async pingDomain(domain) {
        ...
        for (let i = 0; i < this.pingCount; i++) {
            const path =
                this.resources[
                    stat.reqIdx % this.resources.length
                ];
        ...
        const valid = samples
            .filter(
                sample =>
                    ...
                    sample < BeaconEngine.UNAVAILABLE
            )
            .sort((a, b) => a - b);

        if (valid.length === 0) {
            stat.consecutiveTO += 1;
            stat.lastPenaltyCount =
                stat.consecutiveTO;
    ...
    getScore(domain, current) {
        ...
        if (
            ...
            current >= BeaconEngine.UNAVAILABLE
        ) {
            return BeaconEngine.UNAVAILABLE;
        }

        const history =
            Number.isFinite(stat.previousMean)
                ? stat.previousMean
                : current;
    ...
    cancelCurrentRun() {
        ...
        this._activeControllers.forEach(
            controller => {
                try {
                    controller.abort();
                } catch (error) {
